...
# method "B" modified to include a futility criterion for PE and
...
power.2stage.Bf <- function(alpha=c(0.0294,0.0294), n1, CV, GMR,
                            targetpower=0.8, pmethod=c("nct","exact"),
                            usePE=FALSE, powerstep=TRUE, fPElower, fPEupper,
                            ...
                            nsims=1e5, setseed=TRUE, print=TRUE, details=TRUE)
{
  ...
  # fPElower=0 is original Potvin "B"
#  if (fPElower<theta1 | fPEupper>theta2){
...
#  }
  ...
  ltheta1 <- log(theta1)
  ltheta2 <- log(theta2)
  ln_fPElower <- log(fPElower)
  ln_fPEupper <- log(fPEupper)
  lGMR    <- log(GMR)
  mlog    <- log(theta0)
  mse     <- CV2mse(CV)
...
# ----- stage 1 ----------------------------------------------------------
  ...
  outside <- ((pes-ln_fPElower)<1.25e-5 | (ln_fPEupper-pes)<1.25e-5)
  #cat (sum(outside), " studies with PE outside in s1\n")
  BE      <- !outside
  BE[BE==TRUE] <- NA
  ...
  if (powerstep) {
    # method B: evaluate power at alpha[1] and GMR
    ...
    # if not BE and power<targetpower then goto stage 2
  ...
  # combine decision via futility rule and stage 1 CI
  BE[is.na(BE)] <- BE1
  ...
  rm(pes_tmp, mses_tmp)
  # the return list
  res <- list(method="Bf", alpha=alpha, CV=CV, n1=n1, GMR=GMR, 
              targetpower=targetpower, pmethod=pmethod, 
              theta0=exp(mlog), theta1=theta1, theta2=theta2, usePE=usePE, 
              powerstep=powerstep, fPErange=c(fPElower, fPEupper), nsims=nsims,
              # results 
  ...
  if (print) {
    ...
    cat("Futility range for PE= outside ",fPElower," ... ",fPEupper,"\n", sep="")
    ...
    if (powerstep) cat("Interim power analysis step included.\n") else 
      cat("No interim power analysis step.\n")
